...
export default function Services() {
    return (
        <section id="dich-vu" className="w-full py-16 bg-light animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {services.map((s) => (
                        <div key={s.name} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-8 flex flex-col items-center text-center group animate-fade-in">
                            ...
                            <a href={s.href} className="mt-auto px-4 py-2 rounded-full border border-blue-400 text-blue-400 font-semibold hover:bg-blue-400 hover:text-navy shadow transition-all duration-200 focus:ring-2 focus:ring-blue-400 focus:outline-none">Xem chi tiết</a>
                        </div>
                    ))}
                </div>
            </div>
